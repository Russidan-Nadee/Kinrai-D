...
export interface RecommendationStrategy {
  name: string;
  weight: number;
  calculateScore(menu: any, userData?: UserBehaviorData, context?: any): Promise<number>;
}
...
export class RecommendationService {
  ...
  async getAdvancedRecommendations(queryDto: MenuRecommendationQueryDto): Promise<MenuRecommendationResultDto> {
    const startTime = Date.now();
    
    try {
      // Get user behavior data if user_id provided
      const userData = queryDto.user_id ? 
        await this.getUserBehaviorData(queryDto.user_id) : null;

      // Get candidate menus
      const candidates = await this.getCandidateMenus(queryDto);
      
      // Calculate scores using multiple strategies
      const scoredMenus = await this.calculateAdvancedScores(candidates, userData || undefined, queryDto);
      ...
      this.logger.log(`Generated ${finalRecommendations.length} recommendations in ${endTime - startTime}ms`);
  ...
  private async getCandidateMenus(queryDto: MenuRecommendationQueryDto): Promise<any[]> {
    ...
    return this.cacheService.getOrSet(cacheKey, async () => {
      ...
      if (queryDto.excluded_menu_ids?.length) where.id = { notIn: queryDto.excluded_menu_ids };
      if (queryDto.preferred_protein_types?.length) where.protein_type_id = { in: queryDto.preferred_protein_types };
      
      const menus = await this.prisma.menu.findMany({
        where,
        include: {
          Translations: {
            where: this.languageService.createTranslationWhereClause(queryDto.language)
          },
          Subcategory: {
            include: {
              Translations: true,
  ...
  private async calculateAdvancedScores(
    ...
    for (const menu of candidates) {
      ...
      for (const [strategyName, strategy] of this.strategies.entries()) {
        const strategyScore = await strategy.calculateScore(menu, userData, context);
        ...
        subcategory: {
          ...
          category: {
            ...
            food_type: {
              ...
              name: menu.Subcategory.Category.FoodType.Translations[0]?.name || ''
            }
          }
        },
  ...
  private async calculateContentScore(
    ...
    if (menu.protein_type_id && userData.favoriteMenus.length > 0) {
      const proteinPreference = await this.calculateProteinPreference(userData, menu.protein_type_id);
      score += proteinPreference * 0.3;
    }
    
    // Ingredient similarity
    const ingredientScore = await this.calculateIngredientSimilarity(userData, menu.contains);
    score += ingredientScore * 0.3;
    
    return Math.min(score, 1);
  }

  private async calculateTemporalScore(menu: any, userData?: UserBehaviorData): Promise<number> {
    const now = new Date();
    ...
    switch (menu.meal_time) {
      ...
      case 'DINNER':
        ...
        break;
      case 'SNACK':
        mealTimeScore = (hour >= 15 && hour <= 16) || (hour >= 21 && hour <= 23) ? 1 : 0.6;
        break;
    }
  ...
  private async calculateProteinPreference(userData: UserBehaviorData, proteinTypeId: number): Promise<number> {
    // Calculate user's preference for this protein type based on history
    return 0.5;
  }

  private async calculateIngredientSimilarity(userData: UserBehaviorData, contains: any): Promise<number> {
