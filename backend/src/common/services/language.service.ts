...
export class LanguageService {
  ...
  getBestTranslation<T extends { language: string }>(
    ...
    const fallbackPriority =
      LANGUAGE_CONFIG.FALLBACK_PRIORITY[lang] ||
  ...
  async getTranslationsWithFallback<T extends { language: string }>(
    ...
    for (const lang of requestedLanguages) {
      ...
      if (
        translation &&
        !results.find((r) => r.language === translation.language)
      ) {
  ...
  validateLanguage(language: string): SupportedLanguage {
    ...
    if (
      LANGUAGE_CONFIG.SUPPORTED.includes(normalizedLang as SupportedLanguage)
    ) {
  ...
  async getLanguageStats() {
    const stats = await Promise.all([
      ...
      this.prisma.subcategoryTranslation.groupBy({
        by: ['language'],
        _count: { id: true },
      }),
      this.prisma.proteinTypeTranslation.groupBy({
        by: ['language'],
        _count: { id: true },
      }),
    ]);
    ...
    return {
      category_translations: stats[0],
      food_type_translations: stats[1],
      subcategory_translations: stats[2],
      protein_type_translations: stats[3],
